...
titulo<-T_ST_P_No_TRAMOB$TITULO[1]
subtexto<-"Fonte: IBGE"

link <- T_ST_P_No_TRAMOB$LINK[1]
...
data_serie <- paste('[',gsub(' ',',',
                             paste(paste(as.vector(objeto_0[[1]]$`Motocicletas`)),
                                   collapse = ' ')),']',sep = '')
...
texto<-paste('{"title":{"text":"',titulo,
             ...
             '"series":[{"name":"',nomes[6],'","data":',data_serie,',',
...
write(exportJson0,file = paste('data/',gsub('.csv','',T_ST_P_No_TRAMOB$NOME_ARQUIVO_JS[1]),
                               '.json',sep =''))
write(texto,file = paste('data/',T_ST_P_No_TRAMOB$NOME_ARQUIVO_JS[1],
                         sep =''))
