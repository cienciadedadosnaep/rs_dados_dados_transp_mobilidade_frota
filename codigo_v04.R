...
objeto_0 <- dados %>%
  ...
  select(`Ano`,`Total`) %>% #filter(ano<2019) %>%
  #arrange(trimestre) %>%
...
titulo<-T_ST_P_No_TRAMOB$TITULO[5]
subtexto<-"Fonte: IBGE"

link <- T_ST_P_No_TRAMOB$LINK[5]
...
write(exportJson0,file = paste('data/',gsub('.csv','',T_ST_P_No_TRAMOB$NOME_ARQUIVO_JS[5]),
                               '.json',sep =''))
write(texto,file = paste('data/',T_ST_P_No_TRAMOB$NOME_ARQUIVO_JS[5],
                         sep =''))
