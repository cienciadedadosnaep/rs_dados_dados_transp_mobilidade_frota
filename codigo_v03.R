...
dados %<>% mutate(`Caminhões`=round(`Caminhões`/1000,1))
dados %<>% mutate(`Caminhonetas`=round(`Caminhonetas`/1000,1))
...
objeto_0 <- dados %>%
  ...
  select(`Ano`,`Caminhões`, `Caminhonetas`) %>% #filter(ano<2019) %>%
  #arrange(trimestre) %>%
...
titulo<-T_ST_P_No_TRAMOB$TITULO[4]
subtexto<-"Fonte: IBGE"

link <- T_ST_P_No_TRAMOB$LINK[4]
...
data_serie <- paste('[',gsub(' ',',',
                             paste(paste(as.vector(objeto_0[[1]]$`Caminhões`)),
                                   collapse = ' ')),']',sep = '')

data_serie1 <- paste('[',gsub(' ',',',
                             paste(paste(as.vector(objeto_0[[1]]$`Caminhonetas`)),
                                   collapse = ' ')),']',sep = '')
...
write(exportJson0,file = paste('data/',gsub('.csv','',T_ST_P_No_TRAMOB$NOME_ARQUIVO_JS[4]),
                               '.json',sep =''))
write(texto,file = paste('data/',T_ST_P_No_TRAMOB$NOME_ARQUIVO_JS[4],
                         sep =''))
